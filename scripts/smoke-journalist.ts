// Local smoke test for journalist discovery — no DB writes, no API endpoint.
// Exercises the lib code and prints what would be inserted.
//
// Usage:
//   npx tsx scripts/smoke-journalist.ts                       # default: AR × journalist (individuals)
//   npx tsx scripts/smoke-journalist.ts AR journalist
//   npx tsx scripts/smoke-journalist.ts AR journalist sports  # individuals with explicit angle
//   npx tsx scripts/smoke-journalist.ts AR press-union
//   npx tsx scripts/smoke-journalist.ts US press-association
//   npx tsx scripts/smoke-journalist.ts MX press-club
//   npx tsx scripts/smoke-journalist.ts BR journalism-school
...
const category = (process.argv[3] ?? "journalist") as
  | "journalist"
  | JournalistOrgCategory;
const angle = process.argv[4] ?? JOURNALIST_ANGLES[0];

const VALID_ORG: JournalistOrgCategory[] = [
  "press-union",
  "press-association",
  "press-club",
  "journalism-school",
...
console.log(
  `\n═══ Smoke journalist: ${country} × ${category}${isOrg ? "" : ` · angle="${angle}"`} ═══\n`,
);
...
try {
  sonar = isOrg
    ? await searchJournalistOrgs(country, category as JournalistOrgCategory, { maxResults: 12 })
    : await searchJournalistIndividuals(country, { maxResults: 15, angle });
} catch (e) {
...
const kindPrefix = isOrg ? "sonar:journalist-org" : "sonar:journalist-individual";
const cat = isOrg ? category : "journalist";
