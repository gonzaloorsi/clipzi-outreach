...
export const build: TemplateBuilder = ({ channelName, fromName }) => ({
  ...
  html: `<p>Oi ${esc(channelName)},</p><p>Sou ${esc(fromName)}, co-founder da Clipzi (https://clipzi.app/). Transformamos sets, especiais e podcasts de stand-up em clipes prontos para Reels, TikTok e Shorts. Você sobe o vídeo, a IA encontra as melhores tiradas, e finaliza num editor visual.</p><p>Liberamos 2 vídeos grátis por mês para você testar com o próximo material. Se depois quiser mais volume ou funções extras, tem planos pagos.</p><p>Se topar, montamos algo específico para ${esc(channelName)}.</p><p>${esc(fromName)}<br/>Co-founder &amp; CEO, Clipzi</p>`,
});
