...
export const build: TemplateBuilder = ({ channelName, fromName }) => ({
  ...
  html: `<p>Hola equipo de ${esc(channelName)},</p><p>Soy ${esc(fromName)}, co-founder de Clipzi (https://clipzi.app/). Trabajamos con agencias que manejan creadores y necesitan generar volumen de clips para sus clientes.</p><p>Clipzi convierte videos largos en clips listos para TikTok, Reels y Shorts. El equipo sube el video, la IA encuentra los mejores momentos, y luego los ajustan en un editor visual.</p><p>Les abrimos un workspace de prueba con créditos para que prueben el flujo con uno de sus clientes. Si encaja, hablamos plan agencia (multi-team, billing centralizado).</p><p>Si les interesa, podemos hacer algo específico para ${esc(channelName)}.</p><p>${esc(fromName)}<br/>Co-founder &amp; CEO, Clipzi</p>`,
});
