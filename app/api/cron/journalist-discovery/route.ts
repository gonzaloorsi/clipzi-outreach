// Journalist discovery cron — runs every 3h. Iterates (country × kind × category)
// tuples, asks Sonar via AI Gateway for independent journalists with video/audio
// shows (individuals) and journalist organizations (press unions, associations,
// press clubs, journalism schools). Falls back to website scraping when Sonar
// didn't surface an email. Upserts to channels with
// discoveredVia="sonar:journalist-individual:{country}:journalist" or
// discoveredVia="sonar:journalist-org:{country}:{category}".
//
// Individuals rotate through JOURNALIST_ANGLES (beats) by day-of-year — same
// pattern as media-org-discovery — so the same country query surfaces a
// different cluster each day.
//
// Triggered by Vercel Cron (`5 */3 * * *`, 8 ticks/day) or manually via
...
//   ?category=press-union        → restrict to one org category (forces kind=org)
...
import {
  searchJournalistIndividuals,
  searchJournalistOrgs,
  JOURNALIST_ORG_CATEGORIES,
  JOURNALIST_ANGLES,
  type JournalistOrgCategory,
} from "@/lib/journalist-search";
...
// Same 21 markets as standup-discovery: big press ecosystems + our template
// languages. Sonar quality drops on fringe markets, so keep the list curated.
...
function journalistChannelId(kind: Kind, domain: string): string {
  const h = createHash("sha1").update(domain.toLowerCase()).digest("hex").slice(0, 16);
  return `journalist-${kind === "individual" ? "ind" : "org"}:${h}`;
}

interface TupleResult {
  country: string;
  kind: Kind;
  category: string; // "journalist" for individuals, otherwise the JournalistOrgCategory
...
export async function GET(req: NextRequest) {
  ...
  const log = (msg: string) =>
    console.log(`[journalist-discovery ${new Date().toISOString()}]`, msg);
  ...
  for (const c of countries) {
    if (onlyKind === "individual" || (!onlyKind && !onlyCategory)) {
      allTuples.push({ country: c, kind: "individual", category: "journalist" });
    }
    const orgCategories: JournalistOrgCategory[] = onlyCategory
      ? [onlyCategory as JournalistOrgCategory]
      : JOURNALIST_ORG_CATEGORIES;
  ...
  // tuple gets a different beat every day; with 6 angles, the same country
  // cycles through all beats in 6 days.
  const now = new Date();
  ...
  try {
    for (let idx = 0; idx < sliced.length; idx++) {
      ...
      const angle =
        t.kind === "individual"
          ? JOURNALIST_ANGLES[(dayOfYear + idx) % JOURNALIST_ANGLES.length]
          : undefined;
...
async function runOneTuple({
  ...
  try {
    sonar =
      kind === "individual"
        ? await searchJournalistIndividuals(country, {
            maxResults: 12,
            angle: angle ?? JOURNALIST_ANGLES[0],
          })
        : await searchJournalistOrgs(country, category as JournalistOrgCategory, {
            maxResults: 10,
          });
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
  ...
  const discoveredVia =
    kind === "individual"
      ? `sonar:journalist-individual:${country}:${category}`
      : `sonar:journalist-org:${country}:${category}`;
  ...
  const rows = enriched.map((a) => {
    ...
    return {
      id: journalistChannelId(kind, a.website),
      title: a.name,
      cleanName: a.name,
      country,
      language: null,
