// Photographer discovery cron — runs every 3h. Iterates (country × kind ×
// category) tuples, asks Sonar via AI Gateway for photographers/videographers
// who shoot event video (individuals) and photo/video studios + photographer
// associations (orgs). Falls back to website scraping when Sonar didn't surface
// an email. Upserts to channels with
// discoveredVia="sonar:photographer-individual:{country}:photographer" or
// discoveredVia="sonar:photographer-org:{country}:{category}".
//
// Individuals rotate through PHOTOGRAPHER_ANGLES (specialties) by day-of-year —
// same pattern as media-org-discovery — so the same country query surfaces a
// different cluster each day.
//
// Triggered by Vercel Cron (`55 */3 * * *`, 8 ticks/day) or manually via
// Bearer/x-cron-secret. With 21 countries × (1 individual + 2 org) = 63
// tuples, slice rotation processes ~8 tuples/tick keeping each run inside the
...
//   ?category=studio             → restrict to one org category (forces kind=org)
...
// Same 21 markets as standup/journalist discovery. Wedding/event video is a
// global trade, but Sonar quality drops on fringe markets — keep it curated.
...
// Same rationale as standup-discovery: each scrape can take up to ~35s if all
// paths time out, and uncapped scrapes blow past the function cap.
...
function photographerChannelId(kind: Kind, domain: string): string {
  const h = createHash("sha1").update(domain.toLowerCase()).digest("hex").slice(0, 16);
  return `photo-${kind === "individual" ? "ind" : "org"}:${h}`;
}

interface TupleResult {
  country: string;
  kind: Kind;
  category: string; // "photographer" for individuals, otherwise the PhotographerOrgCategory
  angle?: string; // only for individuals
  fromSonar: number;
  domainsTried: number;
  emailsFound: number;
...
export async function GET(req: NextRequest) {
  ...
  const log = (msg: string) =>
    console.log(`[photographer-discovery ${new Date().toISOString()}]`, msg);
  ...
  for (const c of countries) {
    if (onlyKind === "individual" || (!onlyKind && !onlyCategory)) {
      allTuples.push({ country: c, kind: "individual", category: "photographer" });
    }
    const orgCategories: PhotographerOrgCategory[] = onlyCategory
      ? [onlyCategory as PhotographerOrgCategory]
      : PHOTOGRAPHER_ORG_CATEGORIES;
  ...
  // Day-of-year for deterministic angle rotation on individual searches. Each
  // tuple gets a different specialty every day; with 6 angles, the same country
  // cycles through all specialties in 6 days.
  const now = new Date();
  ...
  try {
    for (let idx = 0; idx < sliced.length; idx++) {
      ...
      const angle =
        t.kind === "individual"
          ? PHOTOGRAPHER_ANGLES[(dayOfYear + idx) % PHOTOGRAPHER_ANGLES.length]
          : undefined;
...
async function runOneTuple({
  ...
  log(`tuple: ${country} × ${kind} × ${category}${angle ? ` · angle="${angle}"` : ""}`);

  let sonar;
  try {
    sonar =
      kind === "individual"
        ? await searchPhotographerIndividuals(country, {
            maxResults: 12,
            angle: angle ?? PHOTOGRAPHER_ANGLES[0],
          })
        : await searchPhotographerOrgs(country, category as PhotographerOrgCategory, {
            maxResults: 10,
          });
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
  ...
  const discoveredVia =
    kind === "individual"
      ? `sonar:photographer-individual:${country}:${category}`
      : `sonar:photographer-org:${country}:${category}`;
  ...
  const rows = enriched.map((a) => {
    ...
    return {
      id: photographerChannelId(kind, a.website),
      title: a.name,
      cleanName: a.name,
      country,
      language: null,
